...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['langkawi', 'tioman-island', 'perhentian-islands'],
    'island': ['langkawi', 'tioman-island', 'perhentian-islands'],
    'snorkel': ['perhentian-islands', 'tioman-island', 'langkawi'],
    'dive': ['perhentian-islands', 'tioman-island', 'mulu'],
    'diving': ['perhentian-islands', 'tioman-island'],
    'wildlife': ['sandakan', 'kota-kinabalu', 'kuching'],
    'orangutan': ['sandakan', 'kuching'],
    'borneo': ['kota-kinabalu', 'sandakan', 'kuching', 'mulu'],
    'culture': ['penang', 'malacca', 'kuching'],
    'heritage': ['penang', 'malacca', 'george-town'],
    'food': ['penang', 'kuala-lumpur', 'ipoh'],
    'hawker': ['penang', 'kuala-lumpur', 'ipoh'],
    'relax': ['langkawi', 'tioman-island', 'cameron-highlands'],
    'adventure': ['mulu', 'taman-negara', 'kota-kinabalu'],
    'family': ['kuala-lumpur', 'penang', 'langkawi'],
    'honeymoon': ['langkawi', 'tioman-island', 'perhentian-islands'],
    'romantic': ['langkawi', 'penang', 'cameron-highlands'],
    'budget': ['kuala-lumpur', 'penang', 'ipoh'],
    'luxury': ['langkawi', 'kuala-lumpur', 'penang'],
    'jungle': ['taman-negara', 'mulu', 'sandakan'],
  ...
  return ['kuala-lumpur', 'penang', 'langkawi'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Malaysia AI Trip Planner. You generate detailed day-by-day itineraries for Malaysia travel.

RULES:
- All prices in BOTH MYR (RM) and USD ($). Use rate: $1 = RM4.70
- Include specific restaurant names, hotel recommendations, and transport details
- Use first-person voice: "I recommend...", "you'll love..."
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-kuala-lumpur"
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Beach days in Langkawi"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Malaysia West Coast'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "myr": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Kuala Lumpur'",
      "destination": "string — slug like 'kuala-lumpur'",
      "items": [
        {
          "time": "string — like 'Morning' or '5:30 AM'",
          "description": "string — what to do, with personality",
          "priceMyr": number_or_null,
          "priceUsd": number_or_null,
          "category": "transport|accommodation|activity|food|ferry",
          "affiliateType": "hotel|tour|transport|null",
          "affiliateSlotId": "string_or_null",
          "locationName": "string — specific place name for map pin, e.g. 'Petronas Towers', 'Batu Caves', 'Gurney Drive'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Malaysia trip:\n${parts.join('\n')}`;
