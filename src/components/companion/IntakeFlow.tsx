...
const SAMPLE_PASTE = `Day 1 - Arrive Kuala Lumpur, check in near KLCC
Day 2 - Petronas Towers, Batu Caves, Jalan Alor dinner
Day 3 - ETS train to Penang, George Town street art
Day 4 - Penang Hill, Kek Lok Si Temple, Gurney Drive hawkers
Day 5 - Ferry to Langkawi, Cenang Beach, cable car
Day 6 - Duty-free shopping, mangrove boat tour
Day 7 - Fly home from Langkawi`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Malaysia itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — Peninsular, Borneo, Islands & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 7-day Malaysia trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'peninsular') return t.route.match(/Kuala Lumpur|Penang|Langkawi|Malacca|Cameron/i);
          if (filter === 'borneo') return t.route.match(/Kota Kinabalu|Sandakan|Kuching|Mulu/i);
          if (filter === 'islands') return t.route.match(/Perhentian|Tioman|Langkawi/i);
          return true;
        });
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto' as const }}>
          {['all', 'peninsular', 'borneo', 'islands'].map((f) => (
                  ...
                  <div key={item.id} style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: COLORS.oceanTeal, fontWeight: 600 }}>
                        {item.type === 'transport' ? 'Directions included' : item.type === 'food' ? "Scott's local pick" : 'Local tips included'}
                      </div>
                    </div>
      ...
      {!apiError && (
        <>
          ...
          {[
            ...
            { icon: '💬', title: 'Local Phrases', desc: 'Malay phrases tuned to your destinations with pronunciation guides' },
            ...
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in Malaysia — from Grab drivers to dive masters' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
